...
The workbench trace API is a bounded trace endpoint, not a chatbot endpoint. `Explain Selection` is graph-attached trace and eval infrastructure for a selected node, resource, or source span. It is not an answer endpoint, approved guidance panel, clinical recommendation workflow, or full RAG answer surface. Advice-like prompts and no-source or no-match traces abstain before gateway execution. Validated source-span traces may invoke only the local gateway path, and normal responses expose command, eval, retrieval, source-selection, gateway, model-class, citation-verifier, warning, digest, abstention, and evidence-ID metadata. Raw model output is withheld by default from API responses, UI, evidence, and tests; debug raw output requires both the environment debug flag and request-level debug permission. The current dry-run path is mockable and skippable in CI. The v1 real local runner is opt-in Qwen3 4B through Ollama, guarded by `services/api/app/model_gateway.py`, and may return unavailable, missing-model, timeout, malformed-response, or non-loopback rejection statuses.

Trace results are ephemeral in this milestone. `Explain Selection` does not persist `ModelTrace` records, mutate the graph, write trace payloads to a database, or save trace results to `localStorage` or `sessionStorage`. The UI renders trace metadata only and does not expose chat, prompt, generated answer, raw output, persistence, or mutation surfaces.
