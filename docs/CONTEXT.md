...
Graph-first, source-backed, bounded-agent-assisted guideline lifecycle platform. The center of the product is the guideline graph, not a generic chat interface, whole-corpus generated-answer RAG system, or open PDF question-answering bot. GURU strengthens expert judgment and workflow; it does not replace expert reviewers, working groups, or approval chains.

The Graph-RAG foundation means graph-first traceability before generation. Workbench commands retrieve graph resources, source-span identifiers, path/context nodes, provenance fields, warnings, abstention status, and evidence IDs. Selected-context conversational Graph-RAG v1 may return cited draft answer fragments only when the user has selected exactly one validated source span. It does not provide approved guidance, patient-specific advice, generated clinical summaries, whole-corpus chat, or full RAG answers.
...
- Next.js frontend (`apps/web`) with an Evidence Atlas IDE using Sigma.js and Graphology as the default atlas graph, plus graph-linked retrieval and selected-context assistant surfaces for graph focus, provenance, trace evidence, offline status, source-backed draft answers, and draft review metadata.
...
- A bounded workbench trace and conversation contract for command, eval, retrieval, source-selection, gateway decision, model-class, citation-verifier, abstention, warning, evidence-ID metadata, and selected-context cited draft answer fragments. It is not whole-corpus chat, approved guidance, or patient advice.
- A local, mockable, and skippable `local_open_weight_7b` dry-run path for ModelTrace coverage when validated source-span context exists. It records metadata and abstention status without answer text. The v1 real local runtime is opt-in only: Qwen3 4B through Ollama may run behind the gateway when `GURU_ENABLE_REAL_LOCAL_INSTRUCTION_MODEL=1` and `GURU_LOCAL_INSTRUCTION_PROVIDER=ollama` are set. That runtime is not a permanent model commitment.
...
- Whole-corpus/open-chat answers, generated clinical summaries, or full RAG answers. Only selected-context cited draft answer fragments tied to one validated source span are allowed.
...
The active milestone moves from the preserved metadata catalogue to GURU's graph-linked Evidence Atlas and workbench foundation. Its graph, API, and metadata/source-span retrieval scope uses the 198 public AHS/GURU metadata rows from `resources/registry/ahs-guru-public-corpus.json`. Registry metadata may create resource, disease-site, document-type, and archive/status navigation records. It does not create clinical recommendations, generated summaries, embeddings, all-corpus model answers, or full RAG behavior. Selected-context cited draft answers are allowed only when grounded to one validated source span.
...
The Evidence Atlas UI now uses Sigma.js with Graphology as the default graph model. Its deterministic ForceAtlas and noverlap layout is an implementation detail of the current graph surface. The interface includes resizable/dismissible Vault, Graph Search, Source View, provenance, review, and assistant-rail surfaces; compact provenance and review summaries; readable labels such as `Page 1 · Span 1`; and raw source-span IDs, stable locators, source-document IDs, checksums, and workflow IDs behind details/copy controls. Query results and answer citation chips can focus and highlight graph resources, source spans, path/context nodes, and provenance fields. When the graph does not contain source-span nodes, source-span retrieval hits fall back to the parent resource while keeping source-span IDs, stable locators, checksum/status fields, and reviewer metadata visible where available. The interface remains an atlas and provenance browser for guideline exploration, not a patient-facing answer engine.

The workbench API has two bounded surfaces. `Explain Selection` is graph-attached trace and eval infrastructure for a selected node, resource, or source span. `POST /knowledgebase/corpus/workbench/conversation-turn` is selected-context conversational Graph-RAG v1: it requires exactly one validated `source_span_id`; `selected_node_id` and `resource_id` are display graph metadata only and must not widen grounding. Safe responses may include concise draft answer fragments only when every fragment cites the selected source span through matching citations and graph links. Advice-like prompts, no-source/no-match requests, broad source sets, whole-corpus/open-chat request shapes, gateway denial, gateway-unavailable states, uncited fragments, and unsafe raw-output fields produce no answer fragments. Neither surface is an approved guidance panel, clinical recommendation workflow, patient-advice system, or full RAG answer surface. Validated source-span calls may invoke only the local gateway path, and normal responses expose command, eval, retrieval, source-selection, gateway, model-class, citation-verifier, warning, digest, abstention, answer-link, and evidence-ID metadata. Raw model output is withheld by default from API responses, UI, evidence, and tests; debug raw output requires both the environment debug flag and request-level debug permission. The current dry-run path is mockable and skippable in CI. The v1 real local runner is opt-in Qwen3 4B through Ollama, guarded by `services/api/app/model_gateway.py`, and may return unavailable, missing-model, timeout, malformed-response, or non-loopback rejection statuses.

Trace and assistant results are ephemeral in this milestone. `Explain Selection` and conversation turns do not persist `ModelTrace` records, mutate the graph, write trace or assistant payloads to a database, or save prompts, transcripts, raw model output, answer turns, or conversation history to `localStorage`, `sessionStorage`, or IndexedDB. The assistant rail holds only in-memory turns and reload/remount clears them.

Synthetic typed graph fixtures are test and eval contracts only. Runtime context keeps the real public corpus path: public AHS/GURU metadata plus validated local source spans, stable locators, checksum/status fields, and source-span provenance. Synthetic fixtures must not be described as all-real typed clinical graph extraction.
...
Do not route prompts, documents, or embeddings to external large language model APIs by default. Local, open-weight, or explicitly approved private deployments are the default. Any external routing requires documented approval and a per-use gate. The base policy object sets `external_api_allowed` to `false`; changing it to `true` is a per-use approval, not a default. Current Graph-RAG foundation traces and selected-context draft turns use `local_open_weight_7b` as a model class and dry-run path, not as a hard-coded production model. The v1 real local instruction path is opt-in Qwen3 4B through Ollama behind `services/api/app/model_gateway.py`; it preserves `external_api_allowed=false` and `external_api_used=false` and does not create a generated clinical answer surface. Developers can run `python -m services.api.scripts.qwen_ollama_preflight`, set `GURU_ENABLE_REAL_LOCAL_INSTRUCTION_MODEL=1`, `GURU_LOCAL_INSTRUCTION_PROVIDER=ollama`, `GURU_OLLAMA_BASE_URL=http://127.0.0.1:11434`, and `GURU_OLLAMA_MODEL=qwen3:4b`, pull `qwen3:4b` with Ollama, and then run `python -m services.api.scripts.qwen_ollama_preflight --smoke` for an optional local smoke. CI mocks the Ollama checks and does not require installed Ollama, downloaded Qwen weights, or a running local model.
