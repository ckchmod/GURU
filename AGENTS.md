...
2. Run the test suite for affected modules.
3. Commit the milestone with a clear, scoped message.
4. Push only if a remote is configured and the remote gate is satisfied.

Short form: **update docs/memory → run tests → commit → push only if remote configured**.
...
- Stage only intended files; never stage secrets, PHI, raw large files, or model artifacts.
- Check that a remote is configured before pushing. If no remote is configured, stop after commit and report the state.
